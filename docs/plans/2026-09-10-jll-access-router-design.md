...
Create a shared agent skill that lets a high-capability manager on the Mac delegate operations requiring the corporate VPN, AWS SSO, or JLL SSO to a lower-cost T3 Code worker running on JLLMac. The JLLMac worker may return raw corporate content. Read-only operations run automatically; corporate mutations require user approval.
...
The manager-side skill is the only required skill. JLLMac needs a reachable T3 Code environment through the relay and an inexpensive provider configured to execute delegated work. Worker behavior is supplied in each thread brief, so the first version does not require a companion skill on JLLMac.

## Control plane

Use the standalone `t3chief` CLI as the typed control plane. Configure the JLLMac T3 environment under the stable name `JLLMac`. The skill must verify the environment, project, and live provider catalog instead of hard-coding project IDs, provider instance IDs, model slugs, or credentials.

The first version uses a pull-based result flow:

1. The Mac manager starts a scoped worker thread with `t3chief --environment JLLMac thread start`.
...
This avoids requiring the JLLMac environment to address a manager thread hosted by a different T3 environment. Cross-environment push replies can be added later if the control plane gains an environment-qualified reply address.

## Routing policy

Route an operation to JLLMac whenever it requires any of these capabilities:
...
Keep general reasoning, planning, and synthesis on the Mac. Delegate the smallest useful operation rather than the manager's full conversation. When access requirements are uncertain, perform a read-only connectivity or identity check on JLLMac rather than attempting to authenticate from the Mac.

Discover the cheapest adequate live JLLMac route from `t3chief providers` and available quota data. Do not silently substitute an unverified provider or model.
...
1. Ask the JLLMac worker to inspect current state and prepare the exact proposed mutation.
2. Return the proposal to the Mac manager.
3. Ask the user for approval.
4. After approval, instruct the same JLLMac thread to execute and verify the change.

Credentials, browser sessions, SSO tokens, and VPN configuration stay on JLLMac. Retrieved corporate content and command output may be returned to the Mac without redaction.
...
- If the relay or JLLMac environment is unavailable, report the dependency and preserve the task for retry. Do not run an access-dependent substitute on the Mac.
- If SSO has expired, have the JLLMac worker report the exact login requirement. Do not copy credentials into prompts or logs.
...
- `references/setup.md`: prerequisites for `t3chief`, relay pairing, the `JLLMac` environment name, validation commands, and installation guidance without credentials.
...
- If `t3chief` or the `JLLMac` environment is not configured, leave setup in a clearly reported pending state rather than fabricating a successful remote test.
